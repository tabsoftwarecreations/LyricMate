...
import { supabase, safeFetchSongs, safeInsertFavorite, safeDeleteFavorite, safeFetchFavorites, getLocalFavorites, saveLocalFavorites } from './supabase';
...
export default function LanguageScreen({ route, navigation }) {
    ...
    useFocusEffect(useCallback(() => {
        const fetchData = async () => {
            if (loadedCategoryRef.current !== categoryName) {
                setLoading(true);
            }
            ...
            const favoritesPromise = safeFetchFavorites(currentUserId, 3);

            const [songsResult, favoritesResult] = await Promise.all([songsPromise, favoritesPromise]);

            if (!songsResult.error) {
                setSongs(songsResult.data || []);
                loadedCategoryRef.current = categoryName;
            }
            const localFavs = await getLocalFavorites();
            if (!favoritesResult.error && favoritesResult.data) {
                const serverFavIds = favoritesResult.data.map(f => f.song_id);
                // Local-first merge
                const mergedFavs = Array.from(new Set([...localFavs, ...serverFavIds]));
                await saveLocalFavorites(mergedFavs);
    ...
    const toggleFavorite = async (songId) => {
        ...
        let updatedFavs;
        if (isFav) {
            updatedFavs = favorites.filter(id => id !== songId);
        } else {
            updatedFavs = [...favorites, songId];
        }
        ...
        if (userId) {
            try {
                if (isFav) {
                    ...
                    await safeInsertFavorite(userId, songId);
                }
            } catch (err) {
